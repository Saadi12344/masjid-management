...
function render() {
    ...
    times.forEach(t => {
        ...
        row.innerHTML = `
            ...
            <td>${canManageEntries() ? `<button type="button" class="btn btn-sm btn-primary edit-btn" data-name="${t.name}"><i class="fa-solid fa-pen"></i> Edit</button>` : `<span class="text-muted small">View only</span>`}</td>
        `;
